...
3. **Atomic Packages:** One-to-one mapping. Provides strict isolation and prevents cross-contamination, but introduces operational friction when attempting full-system deployments (requiring manual command chains).
...
Decoupling structural modularity (Filesystem) from deployment logic (Makefile) resolves the trade-offs of the standard taxonomies:
