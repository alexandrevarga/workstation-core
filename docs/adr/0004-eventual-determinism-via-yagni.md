...
The industry standard for Infrastructure as Code (IaC) dictates that a system's state should be 100% deterministic and preemptively mapped. However, attempting to declare every possible configuration of a desktop operating system (Fedora Workstation) introduces immense accidental complexity. It forces the engineer to manage state that may never change or cause issues, violating the YAGNI (You Aren't Gonna Need It) principle and creating maintenance fatigue.

## 2. Alternatives Considered
*   **Total State Management (NixOS / Comprehensive Ansible):** Guarantees absolute determinism but requires mapping thousands of lines of configuration for default behaviors that already work perfectly out-of-the-box. The cognitive load to maintain this exceeds the value for a single-node workstation.
